...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        expr = expr.second
        expr = expr.map(lambda x: scheme_eval(x, env))
        return scheme_apply(procedure, expr, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    store = scheme_eval(expressions.first, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if formals != nil or vals!= nil:
            raise SchemeError("# of args don't match with # of parameters")
        return child
        # END PROBLEM 11
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of args')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        do_define_form(Pair(target.first, Pair(read_line(str(LambdaProcedure(target.second, expressions.second, env))), nil)), env)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        a = Pair(bindings.first.first, a)
        b = Pair(scheme_eval(bindings.first.second.first, env), b)
        bindings = bindings.second 

    check_formals(a)
    return env.make_child_frame(a, b)
    # END PROBLEM 15
